...
- **잘한 점**
  - 처음 사용하는 라이브러리(VueFlow, PDF 생성 등)를 습득하고 실제 기능에 적용함
...
- **배운 점**
  - CQRS와 Facade 패턴을 적용하며, 복잡한 비즈니스 로직을 역할별로 명확히 분리하고, 외부에는 단순한 진입점을 제공하는 설계의 중요성을 배움
  - ERP 시스템 도메인 이해 : 워크 데이 기반 일정 관리 설계, 납기준수율을 반영하며 실무적 도메인 지식 향상
...
- **다음 프로젝트에 적용하고 싶은 점**
  - 배포 환경을 고려한 구조 설계
    → 초기 설계 단계에서 정적 리소스 처리 방식과 배포 환경을 함께 고려하여, 운영 중 발생할 수 있는 이슈를 사전에 예방하고 싶다.
  - 재사용성과 유지보수를 고려한 아키텍처 설계
    → 공통 모듈화, 유틸 함수 분리, 컴포넌트 재사용성을 높이는 설계를 통해 코드 일관성과 생산성을 개선하고 싶다.
  - 프론트엔드의 예외 처리 고도화
    → 프론트에서 에러 메시지가 적절히 처리되지 않아 원인 파악과 커뮤니케이션에 어려움이 있었던 경험을 바탕으로, Axios 인터셉터 또는 공통 에러 핸들러를 도입해 사용자 피드백과 디버깅 효율을 향상시키고자 한다.
